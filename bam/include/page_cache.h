...
struct bam_ptr
{
        ...
        update_page_submit_async(const size_t i, s_ctx &ctx)
    {
        ...
        addr = (T *)array->acquire_page_submit_async(i, page, start, end, range_id, ctx);  // addr是经过转换后的实际内存地址
    ...
    __forceinline__
        __host__ __device__ void
        set_prefetch_val(const size_t i, const size_t prefetch_val)
    {
    ...
    __forceinline__
        __host__ __device__ void
        set_window_buffer_counter(const size_t i, const size_t prefetch_val)
    {
...
struct gids_ptr
{
    ...
    __forceinline__
        __host__ __device__ ~gids_ptr() { fini(); }
...
struct wb_bam_ptr
{
    ...
    __forceinline__
        __host__ __device__ ~wb_bam_ptr() { fini(); }
    ...
    __forceinline__
        __host__ __device__ void
        flush_wb_counter(uint64_t page)
    {
    ...
    __forceinline__
        __host__ __device__ void
        count_mask(uint64_t page, uint64_t *mask_counter)
    {
    ...
    __forceinline__
        __host__ __device__ void
        set_wb(uint32_t *wb_q, uint32_t wb_d, T *pinned_ptr, uint64_t *id_array, uint32_t q_d)
    {
    ...
    __forceinline__
        __host__ __device__ void
        set_time(uint8_t cur_time, uint32_t cur_head)
    {
        time_step = cur_time;
        head_ptr = cur_head;
    }
    ...
    __forceinline__
        __host__ __device__ void
        update_wb(uint64_t page_id, uint32_t reuse_time, uint64_t idx)
    {
    ...
    __forceinline__
        __host__ __device__ void
        update_wb_list(uint64_t page_id, uint64_t reuse_time, uint64_t idx)
    {
    ...
    __forceinline__
        __host__ __device__ void
        update_wb_test(uint64_t page_id, uint32_t reuse_time, uint64_t idx)
    {
    ...
    __forceinline__
        __host__ __device__
            uint32_t
            check_reuse_val(uint32_t page_id)
    {
...
struct page_cache_d_t
{
    ...
    __forceinline__
        __device__
            uint32_t
            wb_find_slot(uint64_t address, uint64_t range_id, const uint32_t queue_, simt::atomic<uint64_t, simt::thread_scope_device> &access_cnt, uint64_t *evicted_p_array,
    ...
    __forceinline__
        __device__
            uint32_t
            wb_find_slot_cpu_agg(uint64_t address, uint64_t range_id, const uint32_t queue_, simt::atomic<uint64_t, simt::thread_scope_device> &access_cnt, uint64_t *evicted_p_array,
                                 uint32_t *wb_queue_counter, uint32_t wb_depth, uint64_t *queue_ptr,
                                 int &evict_cpu, uint32_t &evicted_page_id, uint32_t &queue_reuse, uint64_t *id_array, uint32_t q_depth,
                                 uint8_t time_step, uint32_t head_ptr, uint32_t &evict_time);
    // __forceinline__
...
//     AsyncIOStatus *io_status // ✅ 新增：异步状态参数
// );

// __device__ bool check_data_completion(AsyncIOStatus *io_status);
...
struct range_d_t
{
    ...
    __forceinline__
        __device__
            uint64_t
            wb_acquire_page(const size_t pg, const uint32_t count, const bool write, const uint32_t ctrl, const uint32_t queue,
                            uint32_t *wb_queue_counter, uint32_t wb_depth, T *queue_ptr,
                            int &evict_cpu, uint32_t &evicted_page_id, uint32_t &queue_reuse, uint64_t *id_array, uint32_t q_depth,
                            uint8_t time_step, uint32_t head_ptr);
    __forceinline__
        __device__ bool
        wb_check_page(const size_t pg, uint64_t &page_trans);

    __forceinline__
        __device__
            uint64_t
            wb_cont_acquire_page(const size_t pg, const uint32_t count, const bool write, const uint32_t ctrl, const uint32_t queue,
    ...
    __forceinline__
        __device__ void
        set_cpu_buffer(const size_t index, uint32_t offset);
    __forceinline__
        __device__
            uint32_t
            get_cpu_offset(const size_t index);

    __forceinline__
        __device__ void
        set_prefetch_val(const size_t pg, const size_t count) const;
    __forceinline__
        __device__ void
        set_window_buffer_counter(const size_t pg, const size_t count) const;
};
...
// struct AsyncIOTask
// {
//     uint16_t cid;           // 命令ID（原已有）
//     QueuePair *qp;          // 队列对指针（原已有）
//     uint64_t pc_entry;      // 缓存条目索引（原已有）
//     volatile int completed; // 新增：完成标志（0=未完成，1=完成）
//     uint64_t submit_time;   // 新增：提交时间（用于超时检测）
// };

// async_io.h
// 极简的异步IO状态结构
struct AsyncIOStatus
{
    uint16_t cid;           // 命令ID
    QueuePair *qp;          // 队列对指针
    uint64_t pc_entry;      // 缓存条目索引（原已有）
    volatile int completed; // 完成标志: 0=未完成, 1=完成
    uint64_t start_time;    // 开始时间（用于调试）

    __device__ AsyncIOStatus() : cid(0), completed(0), pc_entry(0), qp(nullptr), start_time(0) {}
    ...
    // 检查是否超时（可选）
    __device__ bool is_timeout(uint64_t timeout_cycles = 1000000) const
    {
        return (clock64() - start_time) > timeout_cycles;
    }
};
...
{
    // 每个QP最新的tail值（用于合并门铃）
    uint32_t last_tail[MAX_QUEUE_PAIRS];

    // 需要写入的门铃列表
    struct
    {
        volatile uint32_t *db_reg; // 门铃寄存器地址
        uint32_t tail_value;       // 要写入的值
    } pending[MAX_BATCH_SIZE];

    std::atomic_int count; // 当前待处理的门铃数
};
...
__forceinline__
    __device__ void
    range_d_t<T>::set_cpu_buffer(uint64_t index, uint32_t offset)
{
    // pages[index].state.fetch_or(DIRTY, simt::memory_order_relaxed);
...
__forceinline__
    __device__
        uint32_t
        range_d_t<T>::get_cpu_offset(const size_t index)
{
    // pages[index].state.fetch_or(DIRTY, simt::memory_order_relaxed);
...
__forceinline__
    __device__ void
    range_d_t<T>::set_prefetch_val(const size_t pg, const size_t count) const
{
    uint64_t index = pg;
...
__forceinline__
    __device__ void
    range_d_t<T>::set_window_buffer_counter(const size_t pg, const size_t count) const
{
    uint64_t index = pg;
    uint8_t p_count = count;
        ...
        case NV_NB:
            ...
            if ((st_new & BUSY) == 0)
            {
                ...
                */
                ...
                // s_ctx temp_ctx;
                // read_data_submit_async(&cache, (c->d_qps) + queue, ((b_page)*cache.n_blocks_per_page), cache.n_blocks_per_page, temp_ctx);
                // read_data_wait_async(&cache, (c->d_qps) + queue, ((b_page)*cache.n_blocks_per_page), cache.n_blocks_per_page, temp_ctx);
    ...
    do
    {
        ...
        case NV_NB:
            ...
            if ((st_new & BUSY) == 0)
            {
                ...
                ctx.member_acquire_ctrl = ctrl;
                // ctx.member_acquire_b_page = b_page;  // 可以不要
                // ctx.member_acquire_c = c;  // 可以不要
        ...
        case NV_B:
            ...
            ctx.isHit = false; // 已经有线程提交IO指令，只需等待，wait内无需再次提交
            {
                ...
                ctx.page_trans = page_trans;
                ctx.observed_page_translation = cache.cache_pages[page_trans].page_translation;
            }
        ...
        range_d_t<T>::acquire_page_wait_async(const size_t pg, const uint32_t count, const bool write, const uint32_t ctrl_, const uint32_t queue, s_ctx &ctx)
{
    ...
    uint64_t index = pg;
    // access_cnt.fetch_add(count, simt::memory_order_relaxed);// 注释
    ...
    do
    {
        ...
        case NV_NB:
            ...
            //     printf("成员函数acquire内执行read_data_wait_async..\n");
        ...
        simt::memory_order_acquire, simt::memory_order_relaxed))
            ...
            Controller *c = cache.d_ctrls[ctrl];
            // c->access_counter.fetch_add(1, simt::memory_order_relaxed);  // submit中已加过一次
...
__forceinline__
    __device__ bool
    range_d_t<T>::wb_check_page(const size_t pg, uint64_t &page_trans)
{
...
__forceinline__
    __device__
        uint64_t
        range_d_t<T>::wb_acquire_page(const size_t pg, const uint32_t count, const bool write, const uint32_t ctrl_, const uint32_t queue,
                                      uint32_t *wb_queue_counter, uint32_t wb_depth, T *queue_ptr,
...
struct array_d_t
{
                                   ...
                                   uint32_t &eq_mask, int &master, uint32_t &count, uint64_t &base_master, s_ctx &ctx) const
    {
        ...
        base_master = __shfl_sync(eq_mask, base_master, master);
        // All lanes in the warp branch on ctx.isHit after submit. Without
        // broadcasting the master's decision, hit rows can observe a stale
        // miss state and keep an invalid addr/start/end tuple.
        ctx.isHit = (__shfl_sync(eq_mask, ctx.isHit ? 1 : 0, master) != 0);
    }
                                 ...
                                 uint32_t eq_mask, int master, uint32_t count, uint64_t &base_master, s_ctx &ctx) const // 最后一个参数: , uint64_t base_master
    {
        ...
        {
            page_cache_d_t *pc = &(r_->cache);
            ctrl = ctx.ctrl; // pc->ctrl_counter->fetch_add(1, simt::memory_order_relaxed) % (pc->n_ctrls);
            queue = ctx.queue;  // 重要，需要与submit的队列号统一
        }
    ...
    __forceinline__
        __device__ void
        wb_coalesce_page(const uint32_t lane, const uint32_t mask, const int64_t r, const uint64_t page, const uint64_t gaddr, const bool write, uint32_t &eq_mask, int &master, uint32_t &count, uint64_t &base_master,
    ...
    __forceinline__
        __device__ void *
        wb_acquire_page(const size_t i, data_page_t *&page_, size_t &start, size_t &end, int64_t &r, uint32_t *wb_queue_counter, uint32_t wb_depth, T *queue_ptr, uint64_t *id_array, uint32_t q_depth,
                        uint8_t time_step, uint32_t head_ptr) const
    {
        uint32_t lane = lane_id();
        r = find_range(i);
    ...
    __forceinline__
        __device__ void
        wb_update_wb(uint64_t page, uint32_t reuse_time, uint64_t idx)
    {
        // TO DO Fix ranges
        auto r_ = d_ranges;
        uint64_t page_trans;
    ...
    __forceinline__
        __device__ void
        wb_update_wb_list(uint64_t page, uint64_t reuse_time, uint64_t idx)
    {
        auto r_ = d_ranges;
        uint64_t page_trans;
        bool cl_find = r_->wb_check_page(page, page_trans);
    ...
    __forceinline__
        __device__ void
        wb_update_wb_test(uint64_t page, uint32_t reuse_time, uint64_t idx)
    {
        // TO DO Fix ranges
        auto r_ = d_ranges;
        uint64_t page_trans;
    ...
    __forceinline__
        __device__
            uint32_t
            wb_check_reuse_val(uint32_t page)
    {
    ...
    __forceinline__
        __device__ void
        set_prefetching(const size_t i, uint8_t p_val)
    {
        auto r = find_range(i);
        auto r_ = d_ranges + r;
    ...
    __forceinline__
        __device__ void
        set_window_buffer_counter(const size_t i, uint8_t p_val)
    {
        auto r = find_range(i);
        auto r_ = d_ranges + r;
    ...
    __forceinline__
        __device__ void
        pin_memory(const size_t i) const
    {
        uint32_t lane = lane_id();
        int64_t r = find_range(i);
        auto r_ = d_ranges + r;
    ...
    __forceinline__
        __device__ void
        unpin_memory(const size_t i) const
    {
        uint32_t lane = lane_id();
        int64_t r = find_range(i);
        auto r_ = d_ranges + r;
...
__forceinline__
    __device__
        uint32_t
        page_cache_d_t::wb_find_slot_cpu_agg(uint64_t address, uint64_t range_id, const uint32_t queue_, simt::atomic<uint64_t, simt::thread_scope_device> &access_cnt, uint64_t *evicted_p_array,
                                             uint32_t *wb_queue_counter, uint32_t wb_depth, uint64_t *queue_ptr,
                                             int &evict_cpu, uint32_t &queue_idx, uint32_t &queue_reuse, uint64_t *id_array, uint32_t q_depth,
                                             uint8_t time_step, uint32_t head_ptr, uint32_t &evict_time)
{
...
__forceinline__
    __device__
        uint32_t
        page_cache_d_t::find_slot(uint64_t address, uint64_t range_id, const uint32_t queue_, simt::atomic<uint64_t, simt::thread_scope_device> &access_cnt, uint64_t *evicted_p_array)
{
    bool fail = true;
    uint64_t count = 0;
    uint64_t global_address = (uint64_t)((address << n_ranges_bits) | range_id); // not elegant. but hack
