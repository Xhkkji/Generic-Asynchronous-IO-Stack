...
inline __device__
bool cq_try_peek_head(nvm_queue_t* cq, uint16_t* cid = NULL, uint32_t* pos = NULL, uint32_t* loc_ = NULL, uint32_t* cq_head = NULL) {
    uint32_t head = cq->head.load(simt::memory_order_relaxed);
    ...
    if (cid != NULL) {
        *cid = static_cast<uint16_t>(cpl_entry & 0x0000ffff);
    }
    if (pos != NULL) {
        *pos = loc;
    }
